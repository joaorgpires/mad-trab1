...
sqrt((POSICAO_X[i]-POSICAO_X[j])*(POSICAO_X[i]-POSICAO_X[j])+
(POSICAO_Y[i]-POSICAO_Y[j])*(POSICAO_Y[i]-POSICAO_Y[j]));
...
PRODUCAO[i] + sum {k in PLANETAS} ENVIAR[k,i] - PROCURA_PLANETA[i];

subject to MIN_CHEGADA {i in PLANETAS} : sum {j in PLANETAS} ENVIAR[j,i] >= PROCURA_PLANETA[i];

subject to MANTER_FLUXO: sum {i in PLANETAS ,j in PLANETAS} ENVIAR[i,j] = 
sum {x in PLANETAS, y in PLANETAS} PROCURA[x,y];
